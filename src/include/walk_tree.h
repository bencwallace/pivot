...
public:
   ...
   * @warning It is not recommended to set balanced=false.
   */
  walk_tree(const std::vector<P> &steps, std::optional<unsigned int> seed = std::nullopt, bool balanced = true);
  ...
  walk_node<P, B, T, Dim> *root() const;

  P endpoint() const override;
   ...
   */
  walk_node<P, B, T, Dim> &find_node(int n);
   ...
   */
  bool try_pivot(int n, const T &r);
   ...
   * @return Whether the pivot was successful.
   */
  bool try_pivot_fast(int n, const T &r);
...
private:
  std::unique_ptr<walk_node<P, B, T, Dim>> root_;
  std::mt19937 rng_;
  std::uniform_int_distribution<int> dist_; // distribution for choosing a random lattice site
  walk_node<P, B, T, Dim> *buf_;            // buffer into which nodes are allocated (used for fast node lookup by id)
};
