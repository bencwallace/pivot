...
walk_node<Dim>::walk_node(int id, int num_sites, const transform<Dim> &symm, const box<Dim> &bbox,
                          const point<Dim> &end, walk_tree<Dim> *tree)
    : id_(id), num_sites_(num_sites), symm_(symm), bbox_(bbox), end_(end), tree_(tree) {}

template <int Dim>
walk_node<Dim>::walk_node(const walk_node &w) : walk_node(w.id_, w.num_sites_, w.symm_, w.bbox_, w.end_, nullptr) {}
...
walk_node<Dim> *walk_node<Dim>::pivot_rep(const std::vector<point<Dim>> &steps, walk_tree<Dim> *tree) {
  ...
  walk_node<Dim> *root =
      new walk_node(1, num_sites, transform(steps[0], steps[1]), box<Dim>(steps), steps[num_sites - 1], tree);
  tree->nodes_[0] = root;
  auto node = root;
...
walk_node<Dim> *walk_node<Dim>::balanced_rep(std::span<const point<Dim>> steps, int start,
  ...
  int id = start + n - 1;
  walk_node *root = new walk_node(id, num_sites, rel_symm, rel_box, rel_end, tree);
...
template <int Dim> walk_node<Dim> walk_node<Dim>::create_leaf() {
  ...
  return walk_node(0, 1, transform<Dim>(), box<Dim>(intervals), point<Dim>::unit(0), nullptr);
}
